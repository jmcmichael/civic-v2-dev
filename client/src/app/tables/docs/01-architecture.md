...
| `filters/enum-icon-select.component.ts`               | The attribute columns' single-glyph enum select (`control: 'icon-select'`)                                                      |
...
|  9  | Header row         | `thead > tr.col-header-row`                | `visibleColumns()`; sorters via `sortOrderFor()` / `onSortChange()`; drag-resize via `nz-resizable` right handles → `onColumnResize()` (boundary transfer between the dragged column and its next resizable neighbor via `resizeColumnWidths`; session `widthOverrides`, cleared by Reset Columns; sort clicks are scoped to the title/caret spans by a capture-phase host listener, so blank-area clicks and the click a drag synthesizes never reach ng-zorro's th-wide sort trigger) |
| 10  | Filter row         | `thead > tr.filter-row`                    | `filterValue(col.key)` / `onFilterChange()`                                                                                                                                   |
| 11  | Enum filter menu   | `cvc-enum-filter-menu` (funnel trigger)    | `col.filter.options` (`CvcEnumOption`); its split Reset clears the column, its `reset-all` item fires `onResetFilters()` (the funnel deliberately stays open); `control: 'icon-select'` renders `cvc-enum-icon-select` here instead — the attribute columns' single-glyph select |
