...
|  9  | Header row         | `thead > tr.col-header-row`                | `visibleColumns()`; sorters via `sortOrderFor()` / `onSortChange()`; drag-resize via `nz-resizable` right handles → `onColumnResize()` (session `widthOverrides`, cleared by Reset Columns) |
