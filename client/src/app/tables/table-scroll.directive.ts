 ...
 * fetch request means, which lets one query pipeline serialise refetches and
 * fetchMores instead of racing them. The app-wide `cvcTableScroll` calls
 * `fetchMore` itself and stays until the browse tables move onto this table.
 *
 * 1. Bottom detection reads the current offset rather than comparing
 *    successive offsets, so dragging the scrollbar to the end in one motion
 *    still fires.
