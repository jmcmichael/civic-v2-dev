...
const requiredValidationMsgFn = (err: any, ffc: FormlyFieldConfig): string => {
  ...
  return etCtrl && etCtrl.value ? `${formatEvidenceEnum(etCtrl.value)} Assertions require at least one ClinGen/CGC/VICC code to be specified.` : 'ClinGen/CGC/VICC Code is required.';
};
...
export const clingenCodeArrayTypeOption: TypeOption = {
  ...
  defaultOptions: {
    templateOptions: {
      ...
      helpText: 'Refers to evidence Classifications from the Standards for the classification of pathogenicity of somatic variants in cancer (oncogenicity) in <a href="https://pubmed.ncbi.nlm.nih.gov/25741868/" target="_blank">Horak et. al. 2022.</a>',
    },
    fieldArray: {
