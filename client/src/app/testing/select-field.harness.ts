...
>(config: TypeGateContractConfig<TField>): void {
  ...
  describe('type-gate contract', () => {
    ...
    it('keeps its value but prompts again when the entity type is cleared', async () => {
      // a value drops only once a chosen type excludes the field; with no type
      // chosen there is nothing to judge it against, and a prepopulated value
      // on a submit form must survive until the curator picks one
      const state = config.formState()
      const h = await mount(state)
      setType(state, config.requiredType)
      await h.settle(0)
