  ...
  implements AfterViewInit
{
  ...
  configureStateConnections(): void {
    ...
    this.state.enums.entityType$.pipe(untilDestroyed(this)).subscribe((enums: CvcInputEnum[]) => {
    ...
    if (!this.optionTemplates) {
      ...
      this.selectOption$.next(this.state.getOptionsFromEnums(this.state.getTypeOptions()))
      return
    }
    this.optionTemplates.changes
      .pipe(withLatestFrom(this.typeEnums$), untilDestroyed(this))
      .subscribe(([tplRefs, enums]: [QueryList<TemplateRef<any>>, CvcInputEnum[]]) => {
        const options = this.getSelectOptionsFn(enums, tplRefs)
        this.selectOption$.next(options)
        this.cdr.detectChanges()
      })
    // selectOption$ populates an ngFor that generates a set of templates,
    // fetched by @ViewChildren() optionTemplates. nz-select options are
    // generated by subscribing to optionTemplate's changes
  }
  getSelectOptionsFn(
    entityTypes: CvcInputEnum[],
    tplRefs: QueryList<TemplateRef<any>>
  ): NzSelectOptionInterface[] {
    return entityTypes.map((entityType: CvcInputEnum, index: number) => {
