 ...
 * table itself is configuration — see `diseases-table.config.ts`.
 *
 ...
 *
 * The legacy `ids` input accepted `number[] | number` (a lone scalar id was
 * never actually passed at either embed site — grepped); narrowed to
 * `number[]` to match every other migrated table.
 *
...
export class CvcDiseasesTableComponent {
  private readonly gql = inject(DiseaseBrowseGQL)
  ...
  protected readonly spec = computed(() =>
    diseasesTableConfig(this.gql, this.cvcTitle(), {
      ids: this.ids(),
    })
  )
}
