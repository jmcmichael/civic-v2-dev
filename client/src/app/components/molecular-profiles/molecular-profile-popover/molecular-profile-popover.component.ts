...
import { Maybe, MolecularProfilePopoverFieldsFragment, MolecularProfilePopoverGQL } from "@app/generated/civic.apollo";
...
export class CvcMolecularProfilePopoverComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.molecularProfileId === undefined) {
      throw new Error("Must pass a molecular profile ID into the molecular profile popover component.")
    }

    this.molecularProfile$ = this.gql.watch({ molecularProfileId: this.molecularProfileId })
      .valueChanges
      .pipe(map(({ data }) => data?.molecularProfile),
        filter(isNonNulled));
  }
}
