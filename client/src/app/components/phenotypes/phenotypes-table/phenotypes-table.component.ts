 ...
 * table itself is configuration — see `phenotypes-table.config.ts`.
 *
...
export class CvcPhenotypesTableComponent {
  private readonly gql = inject(PhenotypesBrowseGQL)
  ...
  protected readonly spec = computed(() =>
    phenotypesTableConfig(this.gql, this.cvcTitle(), {
      ids: this.ids(),
    })
  )
}
